-- Update user roles and add trial system
-- First, drop existing role constraints and update the enum
...
AS $$
  SELECT 
    CASE 
      WHEN p.role = 'TRIAL' AND p.trial_ends_at > now() THEN true
      ELSE false
    END
...
-- Add campaign limits check function
